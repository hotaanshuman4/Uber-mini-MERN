...
export default function Signin() {
  ...
  const handleGoogleSuccess = async (tokenResponse) => {
    try {
      await googleLoginAuth(tokenResponse.access_token);
      navigate('/');
    } catch (err) {
      setError(getApiErrorMessage(err));
    }
    ...
    <div className="min-h-screen bg-black flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          <p className="text-gray-300 text-center mb-8">Sign in to book your ride across India.</p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.button
            ...
            type="submit"
            className="w-full bg-white text-black py-4 rounded-2xl font-bold text-lg flex items-center justify-center space-x-2 hover:bg-gray-100 transition-colors shadow-[0_0_20px_rgba(255,255,255,0.2)]"
          >
            <span>Sign In</span>
            <ArrowRight size={20} />
          </motion.button>
        ...
        >
          <p className="text-gray-400">
            ...
            <Link to="/signup" className="text-white hover:text-yellow-400 font-medium transition-colors">
              Sign Up
            </Link>
          </p>
        </motion.div>
